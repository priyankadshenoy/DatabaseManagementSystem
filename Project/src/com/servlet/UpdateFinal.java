...
public class UpdateFinal extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String selectQuery="SELECT address_id FROM driver_details "
					+ " WHERE D_id=?";
			...
			PreparedStatement preparedStatement;
				...
				String InsertQuery2 = "UPDATE driver_details SET first_name = ?,last_name = ?,SSN = ?,contact_phone = ?,home_phone = ?,address_id = ?,NUID = ?"
						+ " WHERE D_id = ?";
